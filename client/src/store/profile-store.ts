...
export const $profileData=createStore<ProfileData|null>(null).on(fetchProfilefx.doneData,(_,val)=>val.profile)
...
export const $photoLink=createStore<string|null>(null).on(fetchProfilefx.doneData,(_,val)=>baseURL+val.photo)
.on(uploadPhotofx.doneData,(_,val)=>baseURL+val.url)
//Не уверен насчёт этого, попробуй подумать там про фото

export const $myProfile=createStore<boolean>(false).on(fetchProfilefx.doneData,(_,val)=>val.myProfile)

export const $beingEdited=createStore<boolean>(false).on(setBeingEdited,(_,val)=>val).reset(fetchProfilefx)